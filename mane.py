...
# Теперь включаем роутер в диспетчер
        ...
        for participant in participants:
            ...
            try:
                await bot.send_message(
                    ...
                    # ДОБАВЛЕНО: reply_markup чтобы Telegram не скрывал поле ввода
...
# ОБНОВЛЕННЫЙ ХЭНДЛЕР ДЛЯ КНОПКИ АДМИНКИ В ГЛАВНОМ МЕНЮ
...
async def handle_admin_button(message: Message, state: FSMContext):
    ...
    # Устанавливаем состояние админки
    await state.set_state(AdminStates.MENU)
    
    # Отправляем админ-меню с клавиатурой
...
async def go_back(message: Message, state: FSMContext):
    ...
    # ПРЕЖДЕ ВСЕГО: Если мы в админских состояниях
...
async def show_my_bookings(callback: CallbackQuery, state: FSMContext):
    ...
    for i, booking in enumerate(bookings[:10], 1):
        ...
        booking_dt = datetime.fromisoformat(booking_date.replace(' ', 'T')).strftime("%d.%m.%Y")
...
async def handle_any_message(message: Message, state: FSMContext):
    ...
    # Если мы в админских состояниях - не обрабатываем здесь
    if current_state and current_state.startswith("AdminStates"):
        return
