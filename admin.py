...
# === СОСТОЯНИЯ АДМИНКИ (дублируем из main.py для фильтров) ===
...
def register_admin(router: Router, db, bot: Bot, admin_ids: List[int], platform_fee: int = 99):
    ...
    # === ОБРАБОТЧИКИ АДМИНКИ С ФИЛЬТРАМИ СОСТОЯНИЙ ===
    
    @admin_router.callback_query(F.data == CB_ADMIN_MENU, StateFilter(AdminStates.MENU))
    ...
    @admin_router.callback_query(F.data == CB_ADMIN_STATS, StateFilter(AdminStates.MENU))
    ...
    @admin_router.callback_query(F.data == CB_ADMIN_EVENTS, StateFilter(AdminStates.MENU))
    ...
    @admin_router.callback_query(F.data.startswith(CB_ADMIN_EVENTS_DETAIL), StateFilter(AdminStates.EVENTS_LIST))
    ...
    @admin_router.callback_query(F.data == CB_ADMIN_BOOKINGS, StateFilter(AdminStates.MENU))
    ...
    @admin_router.callback_query(F.data.startswith("admin:booking_info:"), StateFilter(AdminStates.BOOKINGS))
    ...
    @admin_router.callback_query(F.data.startswith(CB_ADMIN_BOOKINGS_PAGE), StateFilter(AdminStates.BOOKINGS))
    ...
    # FALLBACK для админских callback'ов
    ...
    async def admin_callback_fallback(callback: CallbackQuery, state: FSMContext):
        ...
        if callback.data.startswith("admin:"):
            ...
            # Пропускаем не админские callback'ы
            pass
