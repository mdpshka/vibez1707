...
from keyboards import *  # Импортируем все клавиатуры и callback-префиксы
...
# РЕГИСТРАЦИЯ АДМИНКИ ПЕРЕД ОСНОВНЫМИ ХЭНДЛЕРАМИ
...
# НОВЫЙ ХЭНДЛЕР: просмотр события из "Мои брони"
@router.callback_query(F.data.startswith(CB_EVENT_VIEW), ProfileStates.MY_BOOKINGS)
async def view_event_from_my_bookings(callback: CallbackQuery, state: FSMContext):
    event_id = int(callback.data.split(CB_EVENT_VIEW, 1)[1])
    ...
    # Специальная клавиатура для просмотра из "Мои брони"
    buttons = []
    ...
    buttons.append([InlineKeyboardButton(text=BTN_BACK, callback_data=CB_NAV_BACK_TO_MY_BOOKINGS)])
...
async def back_from_payment(callback: CallbackQuery, state: FSMContext):
    ...
    # Определяем, откуда пришли - из поиска или из бронирований
    current_state = await state.get_state()
    previous_state = await state.get_state()
    
    # Проверяем, откуда мы пришли
    if previous_state == ProfileStates.MY_BOOKINGS:
        buttons = []
        if not is_confirmed:
            buttons.append([InlineKeyboardButton(text="💳 Забронировать", callback_data=f"{CB_EVENT_JOIN}{event_id}")])
        ...
        keyboard = get_event_details_kb(event_id, callback.from_user.id, is_confirmed)
...
@router.callback_query(F.data.startswith(CB_EVENT_BACK), MainStates.VIEWING_EVENT)
...
@router.callback_query(F.data == CB_PROFILE_MY_BOOKINGS, ProfileStates.VIEWING)
...
@router.callback_query(F.data == CB_PROFILE_MY_EVENTS, ProfileStates.VIEWING)
...
@router.callback_query(F.data.startswith(CB_EVENT_MY), ProfileStates.MY_EVENTS)
...
@router.callback_query(F.data.startswith(CB_EVENT_PARTICIPANTS), ProfileStates.MY_EVENTS)
...
async def handle_unexpected_input(message: Message, state: FSMContext):
    ...
    # Убираем блок для MainStates.MAIN_MENU, чтобы админка работала
    ...
    # Для остальных состояний показываем общее сообщение
    await message.answer(
        UNEXPECTED_INPUT,
        reply_markup=get_back_cancel_kb(),
        parse_mode="HTML"
