...
register_onboarding(router, db, ADMIN_IDS)
...
async def go_back(message: Message, state: FSMContext):
    ...
    # Если мы в состояниях онбординга - обрабатываем возврат в онбординге
    if current_state and "OnboardingStates" in current_state:
        # Обработка возврата в онбординге
        from onboarding import handle_back_in_onboarding
        await handle_back_in_onboarding(message, state)
        return
...
async def callback_fallback(callback: CallbackQuery, state: FSMContext):
    ...
    # Если мы в состояниях онбординга - игнорируем
    if current_state and "OnboardingStates" in current_state:
        await callback.answer()
        return
    
    # Не админские callback'и, которые не обработаны
    try:
...
async def handle_any_message(message: Message, state: FSMContext):
    ...
    # Если мы в состояниях онбординга - игнорируем (онбординг сам обрабатывает свои сообщения)
    if current_state and "OnboardingStates" in current_state:
        # Пропускаем обработку, чтобы хендлеры онбординга могли обработать сообщение
        # Но если хендлеры онбординга не обработали сообщение, оно будет проигнорировано
        # Это нормально, так как онбординг должен обработать все сообщения в своем состоянии
        return
    
    # Проверяем, завершён ли онбординг (только если состояние не установлено)
