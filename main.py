...
# УДАЛЕНО: дублирующий хэндлер для кнопки админки, теперь он в admin.py
...
async def callback_fallback(callback: CallbackQuery, state: FSMContext):
    # ОТЛАДОЧНАЯ ИНФОРМАЦИЯ - ВКЛЮЧИТЬ ЕСЛИ НУЖНО
    # current_state = await state.get_state()
    # logging.debug(f"Fallback callback: data={callback.data}, state={current_state}")
