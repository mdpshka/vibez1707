...
async def handle_admin_button(message: Message, state: FSMContext):
    ...
    # Переходим в состояние админки
    await state.set_state(AdminStates.MENU)
    
    # Используем импортированную функцию из admin.py
    # Для этого нужно передать message как callback
    ...
    await admin_menu_handler(fake_callback)
...
# УБРАН ХЭНДЛЕР handle_unexpected_input - он мешал работе админки
# Вместо него добавлен простой обработчик для непонятных сообщений
@router.message()
