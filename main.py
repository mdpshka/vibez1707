...
@router.callback_query(F.data.startswith(CB_CITY_SELECT), OnboardingStates.CITY)
...
@router.callback_query(F.data.startswith(CB_CITY_PAGE), OnboardingStates.CITY)
...
@router.callback_query(F.data == CB_ONBOARDING_CANCEL, OnboardingStates.CITY)
...
async def admin_access(message: Message, state: FSMContext):
    ...
    await message.answer(ADMIN_MENU_TITLE, reply_markup=InlineKeyboardMarkup(
        inline_keyboard=[
